...
public class CropImageView extends AppCompatImageView {
    ...
    /******************************** ??????λ????????? ************************************/
    private static final float MAX_SCALE = 4.0f;  //??????????????????С???м???????????
    ...
    private float oldDist = 1;          //??????ε????
    private int sumRotateLevel = 0;     //????????90????????
    private float mMaxScale = MAX_SCALE;//??????????????С??????????????????
    ...
    private void initImage() {
        ...
        float transX = mFocusMidPoint.x - (mImageMatrixValues[2] + mImageWidth * mImageMatrixValues[0] / 2);  //X?????λ??
        float transY = mFocusMidPoint.y - (mImageMatrixValues[5] + mImageHeight * mImageMatrixValues[4] / 2); //Y?????λ??
        matrix.postTranslate(transX, transY);
        setImageMatrix(matrix);
        invalidate();
    }

    /** ????????????? isMinScale ?????С??????true ??С????????? false ?????????? */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:  //?????????
                ...
                savedMatrix.set(matrix);  //??????????任??????????????????????б任
                if (oldDist > 10f) mode = ZOOM_OR_ROTATE;//??????????????10????Ч
                break;
...
//                    bitmap.recycle();
    ...
    private File createFile(File folder, String prefix, String suffix) {
        ...
        try {
            File nomedia = new File(folder, ".nomedia");  //????????е????????? .nomedia ???
